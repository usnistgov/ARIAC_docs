...
10. To run a trial use the `run_trial.sh` script. The first argument is the team name which should also be the name of the container. The second argument is the name of the trial to be run. To all trials that are in the trials folder you need to pass a second argument 'runFor example to run the nist_competitor with  `kitting.yaml` trial you would run:
