...
-  However if you need to install packages that are not included in rosdep should create custom build scripts, add those to the competitor_build_scripts directory and include the file name in the `pre_build_scripts` section of the yaml file. These scripts will also need to be added to the Google Drive folder. 
